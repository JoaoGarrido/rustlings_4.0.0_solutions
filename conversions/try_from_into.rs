...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if (tuple.0 > 255) || (tuple.1 > 255) || (tuple.2 > 255) {
            return Err(String::from("Element bigger than 255"));
        }
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if arr.iter().filter(|x| (**x > 255) || (**x < 0)).next().is_some() {
            return Err(String::from("Element out of range"));
        }
        let [r, g, b] = arr;
        Ok(Color{
            red: r as u8,
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            return Err(String::from("Size different from 3"));  
        }
        if slice.iter().filter(|x| (**x > 255) || (**x < 0)).next().is_some() {
            return Err(String::from("Element out of range"));
        }
        Ok(Color{
            red: slice[0] as u8,
